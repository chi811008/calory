...
export async function estimateCalories(
  ...
  // 使用者補充併入同一次視覺呼叫。關鍵:補充是「加法」(修正/新增某項),
  // 不可拿來替換整份判讀 —— 否則使用者一報品名,模型會只列名字提到的食材、
  // 忽略照片裡看得到但沒提到的佐料 (如紅蘿蔔)。
  const trimmed = note?.trim();
  const promptText = trimmed
    ? `${PROMPT}\n【使用者補充】${trimmed}\n` +
      '上述補充是對照片的「額外資訊」(某項的正確名稱、份量,或你看不清的食材),' +
      '用來校正或新增對應項目。請務必仍然列出照片中所有可辨識的食物,' +
      '包含使用者沒提到的配菜、佐料、醬料 (例如紅蘿蔔、生菜)。' +
      '補充只能修正或新增項目,絕不可用來刪除照片裡看得到的食物。'
    : PROMPT;
